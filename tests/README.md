...
| `backup` | no, uses one if given | What a backup can hold, the name it is given, the row parser `content query` needs, a made-up backup packed into a `.zip` and read back out of it, what is inside one, saving a file out of one, a backup of six thousand files opening in under two seconds, the backups in a folder, which files a phone already has, installing a split app out of a zip, Cancel stopping a restore after the first file and stopping the packing, a phone unplugged halfway, and the buttons while it runs - all against a made-up phone. With a phone attached it also takes a real settings-only backup, which only reads |
