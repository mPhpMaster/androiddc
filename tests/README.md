...
| `backup` | no, uses one if given | What a backup can hold, the folder name, the row parser `content query` needs, a made-up backup folder read back, which files a phone already has, and installing a split app - all against a made-up phone. With a phone attached it also takes a real settings-only backup, which only reads |
