...
| `@(Get-Content x \| ConvertFrom-Json)` on a file holding a JSON list | Windows PowerShell hands the whole list back as one array object, so `@( )` wraps it in a list of one: the list of backups read back empty, and a restore of several contacts would have made one contact out of all of them | Assign first, wrap after: `$data = ... \| ConvertFrom-Json`, then `@($data)` |
| `$_.PSObject.Properties['Name']` inside `Where-Object` (Windows PowerShell 5.1) | Falsy where the same expression in an `if` is true, so a filter meant to keep rows dropped every one | Compare it: `$null -ne $_.PSObject.Properties['Name']` |
| `New-Object System.IO.Compression.ZipArchive` after `Add-Type -AssemblyName System.IO.Compression.FileSystem` | *Unable to find type ZipArchiveMode*: `ZipFile` comes from `...FileSystem`, `ZipArchive` and `ZipArchiveMode` from `...Compression`, and the first does not drag in the second. It only showed inside the window - a console session had both loaded already | Load both, and check for a type from each |
| `$list += $item` in a loop over a phone's worth of files | An array cannot grow, so each `+=` builds a new one and copies: 5000 items took 1.4 s, and reading a backup of thirty thousand froze the window for minutes with nothing on screen | Build in `New-Object System.Collections.Generic.List[object]` and hand out `$list.ToArray()`, so callers still get a plain array |
| `@($script:list).Count` where the variable holds a `List[object]` | Throws *Argument types do not match* inside the running window, while `$list.Count`, `[array]$list` and `foreach` over it all work. Measured on the log's line store; three checks failed on the wrapper, not on the thing being checked | Ask the list: `$list.Count`. Keep `@( )` for things that may be a single object or `$null` |
